...
Starting in 0.13.11, file selection, analysis-unit planning, and pattern curation are self-contained prompt-only tasks, so Claude does not reread the repository for inputs already present in the prompt. `learn current` checkpoints analysis output and project-profile commit state; after curation or storage failure, `sync --resume` continues without rerunning completed units. Progress distinguishes AI curation, validation, and pattern-store writes, while token summaries separate uncached tokens from total context processed.
