...
[核心特性](#核心特性) · [快速开始](#快速开始) · [它会生成什么](#它会生成什么) · [Agent 成本建议](#agent-成本建议) · [常用命令](#常用命令) · [Workspace](#workspace) · [文档](#文档)
...
从 0.13.1 开始，`sync` / `learn current` 的文件选择会先经过本地过滤和 gotree 结构索引，再把高价值入口候选交给 AI 做文件选择和单元规划。终端只展示关键阶段和统一的文件选择摘要，候选数量、索引数量、耗时等排查细节写入运行时日志，避免大项目进度行被细节淹没。
