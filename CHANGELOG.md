...
- 修复当前候选与模式库已有记录使用相同 Pattern ID 时被误判为跨能力入口合并的问题；同 ID 当前候选现在作为该 Pattern 的唯一有效版本参与校验和水合。
- 统一规范化校验与水合的来源索引，避免旧记录的能力入口、证据或知识字段污染当前候选，并补充真实 capability 名称的回归测试。

## [v0.20.21]

### 修复

- 修复当前代码学习回放已保存的模式规范化决策时绕过能力入口归属校验的问题；非法合并会自动改为保留独立候选并覆盖检查点，不再使 `sync --resume` 在入库校验阶段失败。
- 补充非法检查点回放的回归测试，确保不同能力入口不会因历史决策被合并为同一条 Pattern。
- 忽略本仓运行时生成的根目录 `.skills-seed/`，避免本地学习状态被发布流程意外提交。
...
- 收紧 `user_context` 的可选上下文表达，缺失时改为显式提示，不再让提示词暗示存在未提供的用户上下文。
- 为当前代码批次增加按焦点的一对一结构约束，确保每个输入焦点都被单独回收并校验，不再允许焦点 ID 漏配或复用后静默通过。
- 补充相关提示词、Schema 和回归测试，统一这条链路的约束表达。
...
- 新增 `skills-seed clear runtime`，用于清理 `.skills-seed/runtime` 下的运行数据；默认只清 runtime，不影响 store 和 cache。
- 为重新分析增加可配置的 runtime 预清理开关，默认开启，但在续跑场景下保守跳过，避免误删正在恢复的运行数据。
- 统一终端进度与运行时输出的语义边界，降低日志、提示词与 AI 结果混杂带来的排查成本。

## [v0.20.15]

### 修复

- 将 `preview files` 的输出改为写入 `.skills-seed/runtime/preview/files/` 的 Markdown 审查页，终端只保留报告路径并标注可删除，便于人工复核完整文件范围。
- 将工作区同步的子项目步骤前缀统一为“学习 / 生成”，让实时进度更清晰，并避免中英文提示词在运行时混用。
- 将 AI 运行时提示词统一为英文自然语言，保留最终 Skills 输出语言由配置控制的分层语义，避免提示词层与产物层混淆。

## [v0.20.14]

### 修复

- 修复 `sync` 中断续跑只完成后续阶段时学习摘要丢失的问题；已入库的模式发现数、保存数和变更范围现在会随恢复状态持久化，续跑完成后不再显示为 0，也不会误判为无变化而跳过生成。
- 为权威规则提取增加章节一对一结构化约束，要求本次输入的每个权威章节都返回覆盖回执，避免 Agent 漏返章节后在权威规则与项目地图刷新阶段失败。
- 补充中断续跑摘要、命令状态提交摘要和权威章节 Schema 的回归测试，并同步更新 Wiki 说明。

## [v0.20.13]

### 修复

- 修复 `patterns update` 未被项目运行时判定表登记的问题；在已初始化项目中执行该命令时，现在会正确加载项目容器并完成模式修订。
- 增加 `patterns update` 的启动判定回归测试，避免命令帮助、运行时初始化和实际执行路径再次不一致。

## [v0.20.12]

### 变更

- 为独立知识审查增加候选回执的一对一结构化约束：当前批次候选 ID 进入动态枚举，决策数量必须与候选数量一致，并继续由本地校验拒绝重复、遗漏和未知 ID。
- 明确审查输出中 `business_method` 与 `revision` 必须处于决策对象同级，避免模型把能力入口错误嵌入修订对象后触发解析失败。
- 收敛生成 Skill 的开发焦点入口，只展示首读参考和代表证据，减少关键词重叠造成的多焦点上下文膨胀。
- 明确 `project-spec`、业务模式和能力入口索引的职责边界，并同步中英文提示词、配置文档、Wiki 与回归测试。

## [v0.20.11]

### 变更

- 增强 Agent 调用失败诊断：归档输出 Schema、错误原因和完整 manifest，并在可重试提示中显示诊断位置。
- 按 Agent 能力生成运行时结构化输出契约；工作区画像和规则 Schema 会约束为当前有效的项目 ID，减少恢复和联动阶段的无效输出。
- 补充重试原因、诊断归档和工作区项目 ID 约束的中英文回归测试。

## [v0.20.10]

### 变更

- 强化当前代码学习的证据焦点流水线：议程只组织可复用的源码责任边界，未分配路径由可配置的覆盖批次安全网处理，并在终端区分开发焦点与覆盖批次。
- 改进中断恢复：议程规划完成后立即持久化，恢复指纹只绑定分析范围、计划契约和显式强制重跑，避免无关配置变化导致重复规划。
- 优化 Rule、Workflow、项目焦点和能力入口的生成路由，补充增量优化、渐进加载和中英文 Wiki、配置、质量实验室回归覆盖。

### 修复

- 修复学习议程覆盖回执、空候选 ID、焦点级知识审查和结构化输出契约的边界校验问题，减少无效知识进入模式库。
- 明确本地源码分析上下文准备阶段不执行 AI 调用，并统一终端提示、运行状态和恢复文案的中英文语义。

## [v0.20.9]

### 变更

- 为 `skills-seed update` 增加真实下载进度：下载阶段显示已下载大小、总大小和百分比，并继续保留查询、校验、安装阶段的动态进度。
- 自更新网络请求增加 2 分钟超时，并明确继承 `HTTPS_PROXY`、`HTTP_PROXY`、`NO_PROXY` 环境变量；网络不可达时不再无限等待。
- 同步更新中英文命令参考、Wiki 和 CLI 帮助，说明下载源、代理配置、进度显示和超时行为。

### 修复

- 修复下载过程只有 spinner、无法判断是否仍在传输的问题；进度事件按时间和字节阈值节流，避免高频刷新造成终端抖动。
...
- 权威规则同步升级为“提取 + 独立复核”的完整两阶段链路：复核 Agent 只重读同一批用户指令来源与初次候选，按不可变章节目录返回完整替换结果；程序继续负责来源、章节、范围与命令策略校验。
- 最终 Skill 的开发焦点路由现在先按已知改动路径筛选，再补充需求语义；多焦点命中时要求并读全部参考并用证据消歧，无法确定时明确报告边界。

### 修复

- 修复入口 Skill 无条件“当前代码优先”可能覆盖用户权威约束的问题。显式规则现在定义可修改边界和命令权限，代码只用于校正陈旧的源码学习摘要。
- 修复构建、CI 与自动化文件可被当作权威命令许可、以及“用户手动执行”被误投影为 `requires_authorization` 的风险；自动化文件不再单独授予命令权限，`forbidden`、`describe_only`、`requires_authorization` 与 `allowed` 的语义在生成入口中明确展示。
- 修复临时覆盖焦点可能进入长期开发焦点导航，以及等价权威规则投影去重可能误删不同约束的问题。

## [v0.20.2]

### 变更

- 简化独立知识审查中的能力入口契约：完整 `business_method` 表示设置或替换，省略即移除，消除状态字段与对象字段互相矛盾的无效输出；审查提示词要求打开直接关联源码中的精确定义，保留完整签名、前置条件和结果语义。

### 修复

- 修复候选证据位于调用方、可复用能力定义位于直接关联实现时被错误拒绝的问题；本地层继续校验完整能力契约和安全的项目相对位置，语义关联由独立源码审查确认。
- 修复 Claude/Codex 可重试错误统一记录为 `ERROR` 却只显示截断原始 JSON 的问题；现在识别结构化输出 Schema 重试耗尽，并在终端与结构化日志中输出本地化的具体原因、调用信息和原始归档路径。

## [v0.20.1]

### 变更

- 扩大项目与工作区 Skill 的 frontmatter 触发语义：保留项目名称作为范围信号，覆盖需求实现、问题排查、业务流程、接口与数据契约、配置、模块边界、Rule/Workflow、生成产物、影响分析和验证任务，不再由项目语言缩窄首次命中范围。
- 为 `.test/quality` 增加可配置的入口 frontmatter 契约，从正文加载前可见的 `name` 和 `description` 独立评测触发完整度、命名和长度；评分引擎不包含特定项目、语言、框架或业务判断。
- 将生成 Skill 的用户 Rule 投影调整到根级 `rules/`，与 `workflows/` 同级，并同步生成计划、链接、文档和回归测试。

### 修复

- 修复失效权威文件符号链接在项目准备阶段被错误报告为“获取当前目录失败”的问题；现在保留权威源路径和原始错误链，并按当前 CLI 语言显示准确的阶段上下文。
- 修复独立知识审查在能力入口已由候选自身完整记录、但该位置未重复出现在普通证据列表时错误拒绝的问题；审查仍禁止引入候选边界之外的位置。

## [v0.20.0]

### 变更

- 以 `docs/ULTIMATE_GOAL.md` 为验收契约重构知识生产流水线，明确分离权威规则提取、项目地图刷新、源码证据学习、独立知识审查、全局规范化和最终 Skill 投影；生产逻辑与提示词不包含特定项目、语言、框架或质量夹具判断。
- 将学习议程升级为证据焦点模型：本地候选完整进入规划，Agent 按源码责任和决策边界组织焦点并返回显式跳过回执；当前分析和 diff 学习共享结构化上下文、证据归属与严格输出契约。
- 独立知识审查改为焦点级串行流水线，同一焦点候选保持在一次 Agent 调用中，每个焦点完成后立即保存 checkpoint，全部焦点通过后再执行一次跨焦点全局规范化；移除固定候选条数切批。
- 新增项目 Rule 的完整维护和生成链路，Rule 与 Workflow 均以 `.skills-seed` 为单一事实源，统一使用 `--content`；同名输入默认在已有内容上由独立提示词增量优化，只有 `--overwrite` 才完整替换。
- 重构生成 Skill 的知识边界与按需加载结构，权威约束、源码知识、能力入口、用户 Rule 和 Workflow 分别投影；删除从源码猜测测试/部署流程、冗余验证矩阵和重复聚合内容，并增加交付就绪检查及生成 manifest。
- 扩充 `.test/quality` 为稳定的 100 分质量评测体系，加入通用多业务 fixture、权威规则覆盖、能力入口、架构关系、证据链接、渐进加载、输出稳定性和生产提示词独立性门禁；测试资源全部保留在 `.test/quality`。
- 统一 Claude/Codex 结构化任务执行、DTO Schema、prompt 输入文件和输出归档，权威章节、学习焦点、知识审查及规范化均使用可校验的一对一回执，并继续保留本地确定性所有权和证据校验。
- 优化 8 阶段终端流水线和中英文 i18n，展示候选准备、议程焦点、隔离分析、焦点级审查、知识入库、权威提取、项目地图和 Skill 就绪状态；CLI 运行错误只输出一次当前语言前缀。

### 修复

- 修复学习规划的跳过回执引用不存在路径、权威规则引用空章节或未知章节时直接阻断同步的问题，从输入收集和结构化归属边界校验真实来源。
- 修复独立知识审查中 `accept`/`reject` 携带无效 revision、能力入口隐式 `keep` 导致不完整回执，以及未经证据证明的安全、原子性、排序或一致性结论可能被保留的问题。
- 修复 API 及项目关键约束可能淹没在画像摘要、业务路由机械聚类、模块依赖自环或错误路径、能力入口索引名不副实和 reference 重复加载的问题。
- 修复 workspace 根 Rule 必须机械影响多个子项目、同名 Rule/Workflow 默认拒绝更新，以及作用范围可能被 Agent 猜测扩大的问题。
- 修复命令权限与 Workflow 安全边界冲突、源码流程被误写为用户工作流，以及生成目录可能被反向学习为权威事实的问题。
- 修复旧命令状态 schema 自动删除恢复信息的问题；不兼容 checkpoint 现在保留原文件并显式报错，由用户决定是否使用 `sync --restart`。
- 修复 Cobra 固定英文 `Error:` 与应用层本地化错误重复输出的问题；中文模式只显示 `错误:`，英文模式只显示 `Error:`。

## [v0.15.5]

### 变更

- 放宽用户工作流优化模板，工作流正文改为按任务选择合适的 Markdown 结构，不再强制固定章节或要求所有工作流包含 Validation / 回滚内容。
- 更新 workflow 命令文档、配置文档、prompt 文档和 CLI Skill 参考，明确工作流内容结构可按任务调整，校验步骤只在相关场景保留。

### 修复

- 修复同名工作流合并时，把章节命名、可选章节或文档内容格式变化误判为冲突的问题；当新输入是在放宽或收窄格式要求时，优先更新工作流正文。
- 修复未提供 `--name` 时，AI 生成中文标题会触发“不安全的标题”并阻断保存的问题；标题继续作为展示名，安全 ID 会从用户输入生成稳定哈希兜底。
- 修复 workspace 模式下仍提示旧命令 `skills-seed add` 的问题，统一改为 `skills-seed workspace add`。
- 修复工作流摘要提取可能把“适用于以下场景：”这类短标签行当作摘要的问题。

## [v0.15.4]

### 变更

- 将当前代码学习的焦点分析彻底改为独立 runtime 调用，并通过共享 runtime 上下文承接议程规划结果，避免串行 batch 复用同一分析会话导致上下文膨胀。
- 新增当前学习模式的 AI 合并优化：AI 只输出候选来源归属和规范化建议，本地规范化服务继续负责字段补齐、覆盖率恢复、严格校验、checkpoint 和入库；AI 失败或输出不合格时自动降级到本地确定性路径。
- 收紧生成 Skill 模板和学习提示词，业务/分类 reference 默认输出路由说明而不是长模式摘要，并明确源码学习结果不是权威硬规则或安全保证。

### 修复

- 修复当前学习后续 batch 因新开或复用会话策略不清而重复读取上下文、分析焦点过多且运行变慢的问题。
- 修复类似 `html.UnescapeString` 这类源码观察可能被学习或生成模板误表达为 XSS 防护结论的问题。
...
- 调整工作区 `sync` 流程：每个子项目按“学习当前代码库 -> 生成该子项目 skills”独立完成，所有子项目处理完毕后再生成工作区根 skills，工作区聚合逻辑继续保持在根项目边界内。
- 优化工作区同步进度输出，降低子项目并发日志噪声，并在刷新行中展示当前子项目、步骤和耗时，便于定位长时间运行或失败的子单元。
- 将当前学习的模式整理从 Agent 提示词迁移为本地确定性归一化、合并和校验流程，删除 `learning-pattern-curate` 提示词和 `--curation-output` 调试出口，减少一次 AI 往返和 schema 漂移风险。
- 改进生成 skills 的知识快照质量：合并重复模式，优先使用仓库证据支持的验证命令，补全测试覆盖缺口提示，并避免输出空洞的通用工具章节。

### 修复

- 修复 Agent 已返回合法 JSON 时仍先进入修复流程，导致嵌套对象被误提取为顶层结果并触发 `unknown field "needed"` 或 `unknown field "profile_refresh_recommended"` 的学习失败问题。
- 升级 `github.com/silaswei-io/jsonrepair-go` 到 `v1.1.2`，并在本地 JSON 解析入口增加合法 JSON 快路径，确保正常结构化输出按原始 JSON 严格解析。
- 修复工作区断点续跑时子项目流程状态与终端提示不一致的问题，避免看起来全新开始但实际复用子项目学习记忆。
...
- 将当前代码学习重构为独立 runtime 调用流程：候选收敛、学习议程规划、证据焦点分析、增量 diff 分析和项目画像同步分别使用单次结构化调用，跨阶段上下文通过 runtime 文件显式传递，避免长会话上下文膨胀。
- 重写学习提示词架构，统一 `learning-*` 与 `core-*` 命名，强化源码证据、业务模式、代码模式、边界说明、召回优先和反泛化要求，并删除无状态学习、旧分析单元、fix/check/review/history 等非核心提示词与命令路径。
- 引入统一 JSON 解析工具，所有 Agent 结构化输出先从文本中提取并修复 JSON，再按目标 schema 严格解析，兼容 Claude/Codex 返回 Markdown 代码块、前置说明或局部格式错误的情况。
- 收敛内部包架构：删除含义不清的 `internal/pkg`，将日志和进度移动到 `internal/terminal`；将项目路径能力移动到 `internal/projectpath`；将源码分类移动到 `internal/sourcecode`；将知识展示子包合并为 `internal/knowledge`。
- 将 Go 版本提升到 `go 1.26`，并同步更新配置、命令、提示词和 Skill 模板文档。

### 修复

- 修复 Agent 成功返回但 structured output 被包在普通文本或 Markdown JSON 代码块中时学习失败的问题。
- 修复终端阶段输出滞后、AI 候选收敛长时间无可见进度，以及失败后诊断信息不够直接的问题。
- 修复增量学习和工作区学习中焦点编号、重复焦点、项目画像刷新和模式策展边界不清导致的重复分析或覆盖回退问题。
- 修复生成 Skill 中部分覆盖警告、知识声明和验证建议文案绕过 i18n、学习结果被误表达为权威硬规则、以及模板/代码结构说明不一致的问题。
- 删除 token 费用/用量计算与相关冗余状态，避免不准确统计影响终端输出和架构复杂度。

## [v0.14.2]

### 变更

- 统一 Agent CLI 重试流程，Claude/Codex 共享重试、进度恢复和可重试错误判断，减少运行时分支差异。
- 收敛 Agent Token 消耗展示：终端、诊断字段和运行时 manifest 只输出 Token 统计，不再输出费用明细。
- 改进工作区关系学习的候选归一化，配置继续作为项目身份真源，AI 输出中的项目路径别名、外部系统名称和无效路径会在进入严格校验前规范化或丢弃。

### 修复

- 修复工作区画像学习中平台名、镜像仓库、Jenkins URL 或不存在根路径被写入 `shared`/`contracts`/`infra` 后导致学习失败的问题。
- 修复 workspace 默认路由与影响路由重复、子项目路径别名导致未知项目、以及工作区路径消费者/生产者合并不完整的问题。
- 修复仅保存工作流但尚无模式时生成提示不准确的问题。

## [v0.14.1]

### 变更

- 移除质量不足且扩大架构复杂度的 `local`/`hybrid` 学习后端，当前代码、历史提交、项目画像和模式策展恢复为一致的 Agent 学习链路；CodeGraph 继续作为结构化源码分析与符号校验能力使用。
- 将项目工程规则建模为带来源和证据的独立知识类型，从仓库级说明文件提取可执行规则，并在生成 Skill 时与源码学习模式明确分层。
- 重构工作区画像和规范边界：配置负责项目身份，Agent 只负责关系事实与候选指导；项目引用改为带类型的强类型引用，系统规则、用户规则和分析建议分别表达。
- 统一工作区子项目 Skill 目标解析，学习与生成共享同一条配置解析和旧默认路径规范化链路。

### 修复

- 修复本地学习模式触发空指针崩溃、少量低质量模式以及历史学习语义退化的问题，并删除对应的本地分析、确定性策展和源码采集冗余代码。
- 修复恢复状态只保存简化文件输入而丢失哈希算法、时间、来源、筛选原因和分析状态的问题；未完成运行现在可基于完整文件记录可靠恢复。
- 修复工作区关系指纹发生变化时仍跳过分析、AI 结果覆盖配置默认路由或系统规则，以及未知项目引用延迟到生成阶段才暴露的问题。
- 修复子项目路由仍可能指向旧 `skills-seed-skills` 目录、规则证据路径通过符号链接越出项目根目录，以及无测试文件的 Go module 缺少基础编译校验命令的问题。
...
- 将 CodeGraph 索引生命周期、结构化上下文和源码符号解析收敛为共享基础设施；`auto` 默认稳定使用并自动维护 CodeGraph，只有显式配置 `treesitter` 时才使用内嵌解析器，同时升级 gotreesitter 以修复深层语法树解析的栈溢出问题。
- 重构当前代码与工作区学习为有状态运行对象。分析单元结果逐批写入 command cache，恢复时从首个未完成单元继续；项目日志、worker 日志作用域和工作区并发错误现在保留真实来源。
- 将文件指纹和源码快照提交延后到全部单元分析、项目画像、模式策展及模式库写入成功之后，避免中断后把未入库结果误标记为已学习。
- 模式策展新增候选集合绑定的决策 checkpoint，并支持 `sync --resume --curation-output` 导入既有 Claude/Codex 结构化归档；本地校验或入库失败后可直接重放，不再重复消耗 AI Token。
- 源码位置验证统一通过可切换的 symbol resolver，生成阶段复用相同 CodeGraph/tree-sitter 事实目录，减少学习、校验和 Skills 生成之间的解析差异。

### 修复

- 修复 AI 策展对同一 source 同时保留、合并和丢弃时产生重复所有权、重复 dropped 决策或覆盖缺口的问题；同 ID 输出优先拥有自身来源，无明确归属的冲突交给本地确定性恢复，最终严格校验保持不变。
- 修复工作区某个子项目失败后返回兄弟任务取消错误、项目最终错误未写入对应日志、worker goroutine 丢失日志 scope，以及已完成项目再次运行仍重新筛选、规划和分析的问题。
- 修复分析计划未完整覆盖过滤后代码文件时仍提前进入项目画像、部分单元完成即提交文件状态，以及恢复状态与当前调用参数不一致时复用旧计划的问题。
- 修复文件筛选和分析单元规划无法读取按路径引用的 runtime 输入或必要仓库源码，以及生成阶段重复解析源码、验证符号来源不一致的问题。
...
- Agent CLI 触发可重试的速率限制或过载错误时，终端现在会稳定输出包含等待时间和 API 原因的重试提示，避免动态进度行刷新后丢失具体原因。
- Claude / Codex 的可重试调用失败不再打印成最终的 “CLI 调用失败” 警告，避免后续重试成功后仍让用户误以为本轮 AI 文件筛选失败。

## [v0.13.8]

### 变更

- 提示词学习、整理和当前代码分析流程增加共享抽象层级规则，统一区分事实、局部实现、项目模式和工作区规则，减少弱事实或代码摘要进入 pattern。
- 当前代码学习、项目画像、工作区画像、工作流整理和修复生成 prompt 补充稳定排序、证据边界和抽象层级说明，提高重复运行时的输出稳定性和可路由性。

### 修复

- 运行时 prompt / Agent 输出归档继续使用秒级时间戳，并在同一秒内追加递增序号，避免重复任务覆盖调试文件，同时不引入微秒或纳秒级文件名。
- 修复显式传入 runtime ID 时仍会额外消耗自动 ID 序号的问题，避免后续自动生成的 runtime 文件名出现无意义跳号。
- 用户手工定义 pattern 的 prompt 不再追加代码学习专用抽象规则，避免把无源码证据的人工约定误判为不可沉淀。
...
- 运行时 AI 输出契约增加最终自检要求：模型在返回前必须同时检查 JSON 可解析性和目标语言一致性，若自然语言字段不符合 `skills.locale`，需要先在内部翻译或重写后再输出，避免英文 Skills 中混入中文 learned pattern。
- 语言自检明确只约束用户可见自然语言字段；任务要求原样复制的代码片段可以保留源码注释和字符串字面量，避免破坏证据片段。
...
- `learn current` / `sync` 的文件筛选和分析单元规划会优先使用 CodeGraph 结构化上下文；缺少索引或索引异常时会自动初始化、同步或修复，只有真正不可用时才降级到内嵌 tree-sitter。
- 分析规划和项目画像 prompt 不再内联展开完整候选/焦点文件列表，改为将路径清单去重、排序后写入 runtime 输入文件，并在 prompt 中只引用文件路径和数量，减少大项目 prompt 噪音。
- 文件筛选、分析单元规划和当前代码学习 prompt 增加稳定决策规则；最终 JSON 输出契约也补充通用稳定排序约束，降低相同输入下的随机漂移。

### 修复

- 移除 AI 文件筛选固定数量预算残留，保留结果完全由相关性建议和本地校验决定，避免大项目被意外收敛到固定文件数。
- 收敛本地过滤、AI 文件筛选和文件指纹提交的终端输出，只展示源数量、过滤数量和最终数量等关键关系，详细诊断继续写入 runtime 日志。
- 将 AI 文件筛选前的结构化上下文收集拆到独立边界，避免 tree-sitter 在无明确 seed 时触发全仓扫描，并让结构化上下文传递链路可测试。
...
- 收敛语言配置职责：`locale` 只负责工具输出、配置模板和 seed context 模板语言；`skills.locale` 统一负责运行时 AI 输出契约、学习沉淀内容和生成 Skills 语言，避免工具语言与 Skills 内容语言交叉混用。
...
- 收敛生成 skill 名称与 workspace 子项目 skill 路径解析规则，避免 generator / workspace 重复维护命名逻辑，并防止 workspace 学习输入读取阶段创建子项目默认配置。
...
- `learn current` / `sync` 的文件选择拆分为本地过滤、gotree 结构索引、gotree 文件确认、AI 文件选择和 AI 单元选择等清晰阶段，终端进度只展示关键阶段，细节指标写入日志和统一摘要。
- 大型项目的 AI 文件选择现在使用 gotreesitter 结构索引生成高价值入口候选，避免把超长文件树直接塞进 Agent prompt，并减少不同运行间分析文件数量的大幅波动。
- 运行时 AI JSON 输出契约改为由 Go DTO 生成，提示词不再手写易漂移的 JSON 字段说明，降低模型输出字段类型和解析结构不一致的风险。
- 将初始化生成的长期上下文模板移到 `embedfs/templates/seed/context/`，让运行时 prompt、种子模板和生成 Skills 模板职责更清晰。

### 修复

- 修复多个学习/画像/生成 prompt 中 JSON 字段说明与 Go 解析结构不一致的问题，包括验证命令证据、画像增量、位置字段和批量单元输出。
- 修复恢复上次分析计划时终端摘要口径不清晰的问题，现在展示源文件数量、本地过滤后计划输入、发送给 AI 文件选择、AI 最终选择、恢复后待分析文件和分析单元。
- AI 输出解析继续保留 `jsonrepair-go` 作为通用 JSON 修复能力，但移除额外补丁式兼容解析，失败从源头通过契约和提示词约束修复。
