...
- 新增本地 `.test` 质量验收工具，以 `front_gateway` 作为业务基准项目，检查生成链路、解析稳定性、参考路由、业务模式覆盖、源码证据和导航精度。
- `front_gateway` 业务金标准覆盖 TP1 证书签发、TP3 业务解密/GAS/AEP 链路、异步任务、HSM 会话池、AK/SK 鉴权与外部服务边界。
