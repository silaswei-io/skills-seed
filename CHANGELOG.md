...
- 将当前代码学习重构为分段会话式流程：候选收敛、学习议程规划、证据焦点分析、增量 diff 分析、项目画像同步和全局模式策展分别使用短会话，避免长会话上下文膨胀，同时保留证据包内的多轮提纯能力。
...
- 修复增量学习和工作区学习中会话缓存、焦点编号、重复焦点、项目画像刷新和模式策展边界不清导致的重复分析或覆盖回退问题。
