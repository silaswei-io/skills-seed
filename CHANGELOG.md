...
- `init --skills-name <name>` 和交互式初始化支持指定 Skill 名称，保存为 `skills.name`，统一用于生成入口与输出目录；显式名称不追加 `-dev`，工作区根与子项目保持独立命名。
- 增加名称合法性校验，在写入配置或重置备份前拒绝路径分隔符、非法字符与超长名称；工作区路由读取子项目实际配置的名称和路径。

### 优化

- 将焦点源码分析与独立知识审查拆成可分别恢复的阶段，分析成功立即保存候选和事实材料；审查失败后续跑只补审查，并保存其他在途分析的成功结果，减少重复调用。
- 源码分析与串行审查流水执行并共享 Agent 调用额度，workspace 子项目不再放大根级并发；审查使用独立会话，复用源码路径、diff 引用与结构事实。
- 明确焦点独占的学习责任与可共享的只读证据边界；无关联候选跳过 Agent 规范化，单一来源保留已审查的身份、文本和置信度，规范化检查点绑定相关知识和用户上下文。
- 结构化结果失败最多执行一次携带前次输出及校验原因的修复；排队、执行、退避和修复共享阶段超时预算，避免重试不断延长运行。

### 升级说明

- checkpoint 格式升级为 schema 5，不迁移旧执行状态；升级后若有旧的未完成运行，请使用 `skills-seed sync --restart`。

### 测试与文档

- 补充阶段恢复、取消与并发预算、结构修复、规范化保真、自定义名称持久化和工作区边界回归测试；同步更新中英文命令、配置、快速开始与学习排障 Wiki。

## [v0.20.33]

### 修复

- 统一以消解符号链接后的真实路径校验 Skill 输出目录和 Markdown 本地链接，拒绝配置到项目外的生成目录、构建期间被替换为外部符号链接的输出目标，以及逃逸生成 Skill 根目录的链接。
- 使用 Go 语法解析识别 CommonMark 误判为链接的泛型、索引和调用表达式；复杂源码表达式不再触发假阳性，同时相邻的真实链接仍会执行完整性检查。

### 测试

- 补齐 CLI 入口、重试生命周期、文件变更分类、知识证据核验、领域模型、输出 manifest、仓储和同步流程等关键分支，将全仓语句覆盖率提升到 75.3%。
- 增加路径逃逸、符号链接切换、编码与文件系统错误、取消恢复及中英文输出等回归用例，并通过 race、vet 和 staticcheck 检查。
...
- 明确生成 readiness 会校验真实本地 Markdown 引用，同时忽略行内代码、代码围栏和源码索引表达式，便于区分生成内容问题与校验器误报。
...
- 修复 `sync --resume` 仍进入普通候选筛选和议程规划阶段、造成看起来从零开始的问题；恢复运行现在直接复用已保存的候选与议程，只处理尚未完成的证据焦点。
- 修正恢复诊断中的待处理数量，区分输入文件、已完成焦点和剩余焦点，避免进度统计误导。
- 强化 AI 结构化结果的统一边界校验，拒绝空 verdict、非法 reason code、缺失字段、越界置信度和无效知识标记，并补充跨运行恢复与解析回归测试。
- 修复生成 readiness 将源码索引表达式（如 `params["deadline"](RFC3339Nano)`）误识别为 Markdown 链接的问题，继续校验真实本地链接并忽略代码片段中的链接样式。

## [v0.20.27]

### 变更

- 优化 workspace `sync` 的子项目进度终态，分别显示学习变化统计、无学习变化并跳过生成，以及无学习变化但补全缺失 Skills，不再使用含糊的“完成”或带全局前缀的跳过文案。
- workspace 顶层学习结果现在聚合全部子项目的文件变化、候选模式、规范化写入和移除数量，并同步写入终端汇总、changelog 与 sync 运行记录；工作区关系未变化不再掩盖子项目学习产出。
- 补充工作区同步结果与中英文统计文案的回归测试，并同步更新配置文档和中英文 Wiki。

## [v0.20.26]

### 修复

- 统一学习统计文案，将本轮审查产出明确显示为“发现候选模式”，将包含相关既有 Pattern 的最终 upsert 数显示为“规范化写入”，避免出现“发现 5 个、保存 7 个”时被误解为超额新增。
- 同步修正分析结果、实时 `sync` 摘要、learn/sync 运行记录和 changelog 明细的中英文表述，并补充国际化回归测试。
...
- 修复当前源码全量与增量分析只在提示词中声明焦点约束、却未将焦点 ID 传入 Claude/Codex 运行时 Schema 的问题；实际供应商调用现在会按本批输入收紧焦点枚举与结果数量。
- 修复增量候选被本地证据准入拒绝后整条焦点决策一并丢失的问题；没有其他合格决策时会保留唯一的 `no_change` 回执，不再把“无可入库模式”误报为“遗漏证据焦点”。
- 补充 Claude、Codex 实际 CLI Schema 与增量准入降级的回归测试，覆盖 `api-contract-design` 失败场景。
