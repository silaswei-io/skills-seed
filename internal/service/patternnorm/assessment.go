...
type normalizationCoverage struct {
	CandidateCount int
	MissingIDs     []string
}

func (c normalizationCoverage) MissingCount() int {
	return len(c.MissingIDs)
}

func (c normalizationCoverage) MissingRatio() float64 {
...
type normalizationAssessment struct {
	Result                       *proposal
...
func assessNormalization(result *proposal, candidates, existing []domain.Pattern) normalizationAssessment {
	assessment := normalizationAssessment{Result: cloneProposal(result)}
