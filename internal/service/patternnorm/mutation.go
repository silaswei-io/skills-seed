...
func buildMutationPlan(normalized []domain.Pattern, dropped []Drop, existing []domain.Pattern, intent mutationIntent) mutationPlan {
	plan := mutationPlan{Written: make([]domain.Pattern, 0, len(normalized))}
	outputIDs := make(map[string]struct{}, len(normalized))
...
func patternForSave(pattern domain.Pattern) domain.Pattern {
	pattern = patternview.Normalize(pattern)
	now := time.Now()
