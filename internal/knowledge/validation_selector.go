...
func (selector commandSelector) Choose(area ValidationArea) commandChoice {
...
func (selector commandSelector) chooseByEvidenceForArea(area ValidationArea, narrowOnly bool) commandChoice {
	best := commandChoice{}
	bestScore := 0
...
func (selector commandSelector) chooseByText(area ValidationArea) commandChoice {
	best := commandChoice{}
	bestScore := 0
	evidenceRoots := validationPathRoots(area.Evidence)
	for _, command := range selector.commands {
...
func commandText(command ValidationCommand) string {
...
func commandCoverageAllowed(command ValidationCommand, coverage float64, evidenceCount int) bool {
...
func commandLooksHeavy(command ValidationCommand) bool {
...
func commandLooksBroadTest(command ValidationCommand) bool {
...
func commandIsNarrow(command ValidationCommand) bool {
...
func commandTypeScore(command ValidationCommand) int {
...
func commandTypeScoreForArea(command ValidationCommand, area ValidationArea) int {
...
func commandAppliesToArea(command ValidationCommand, area ValidationArea) bool {
	if area.Kind == "" {
		return true
	}
	commandKind := commandKind(command)
...
func commandKind(command ValidationCommand) string {
	switch ValidationCommandKind(command) {
...
func commandScopePaths(command ValidationCommand) []string {
	return commandDeclaredScopePaths(command)
}

func commandDeclaredScopePaths(command ValidationCommand) []string {
	...
	return stringx.UniqueNonBlank(paths)
}
