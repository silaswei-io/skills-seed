...
func ClaimGroups(patterns []domain.Pattern, locale string) []ClaimGroup {
	if len(patterns) == 0 {
		return nil
	}
	groups := []ClaimGroup{
...
func ClaimFromPattern(pattern domain.Pattern, locale string) Claim {
	decision := EvaluatePatternClaim(pattern)
	return Claim{
...
func claimText(pattern domain.Pattern) string {
	return DisplayPatternText(pattern)
}

func usage(strength ClaimStrength, locale string) string {
	switch strength {
	case ClaimStrengthRule:
		return i18n.GetForLocale(locale, "KnowledgeClaimUsageRule")
	case ClaimStrengthSolution:
...
func strengthIndex(strength ClaimStrength) int {
	switch strength {
	case ClaimStrengthRule:
		return 0
	case ClaimStrengthSolution:
