...
func TestPrintConsoleLineDefersActiveProgressLineConsoleOutput(t *testing.T) {
	output := captureStdout(t, func() {
		...
		PrintConsoleLine("后台消息: 本次 575.5k")
	...
	messageIndex := strings.Index(output, "后台消息: 本次 575.5k")
	if messageIndex < 0 {
		t.Fatalf("expected deferred console output, got %q", output)
	}
	if messageIndex < completedIndex {
...
func TestPrintConsoleLinePreservesActiveProgressLine(t *testing.T) {
	output := captureStdout(t, func() {
		...
		PrintConsoleLine("后台消息: 本次 120.1k")
	...
	messageIndex := strings.Index(output, "后台消息: 本次 120.1k")
	if messageIndex < completedIndex {
		t.Fatalf("expected console output after completed progress line, got %q", output)
	}
}
...
func TestRenderClipsLongProgressLineToTerminalWidth(t *testing.T) {
	...
	output := captureStdout(t, func() {
		...
		tracker.enabled = true
		tracker.active = true
		tracker.label = strings.Repeat("很长的业务证据焦点", 10)
		tracker.startedAt = time.Now()
...
func TestUpdateStepPrintsDetailWhenProgressDisabled(t *testing.T) {
	output := captureStdout(t, func() {
		...
		tracker.UpdateStep("分析当前代码库 · 焦点 2/17 · registry-management")
	})
...
func TestPrintConsoleLineAfterProgressPrintsAfterCompletedStep(t *testing.T) {
	output := captureStdout(t, func() {
		...
		tracker.StartStep("分析当前代码库")
		PrintConsoleLineAfterProgress("后台消息: 本次 575.5k")
		tracker.CompleteStep("分析当前代码库")
	})
	...
	messageIndex := strings.Index(output, "后台消息: 本次 575.5k")
	if messageIndex < 0 {
		t.Fatalf("expected deferred background output, got %q", output)
	}
	if messageIndex < completedIndex {
		t.Fatalf("expected deferred background output after completed progress line, got %q", output)
	}
}

func TestPrintConsoleLineAfterProgressPrintsImmediatelyWithoutActiveStep(t *testing.T) {
	output := captureStdout(t, func() {
		PrintConsoleLineAfterProgress("后台消息: 本次 120.1k")
	})

	if output != "后台消息: 本次 120.1k\n" {
...
func TestMultiTrackerFailMarksTaskAndFlushesPendingLines(t *testing.T) {
	output := captureStdout(t, func() {
		...
		PrintConsoleLineAfterProgress("后台消息: 子项目 backend")
		tracker.Complete("backend", "完成")
	...
	if !strings.Contains(output, "后台消息: 子项目 backend") {
