...
// FindSeedPath 从当前目录向上查找 .skills-seed 目录。
...
// LoadConfig 加载 .skills-seed/config.yaml，不创建 Container。
...
// Resolve 按项目根目录解析输出/配置路径，并展开 "~"。
func Resolve(projectRoot, path string) (string, error) {
	if path == "" {
...
// ResolveOutput 解析项目输出路径，并确保输出目录不逃出项目根目录。
func ResolveOutput(projectRoot, outputPath string) (string, error) {
	resolvedPath, err := Resolve(projectRoot, outputPath)
	if err != nil {
		return "", err
	}
	if projectRoot == "" {
	...
	pathAbs, err := ResolveWithinRoot(projectRoot, resolvedPath)
	if err != nil {
...
func ResolveWithinRoot(root, target string) (string, error) {
	_, err := CanonicalWithinRoot(root, target)
	if err != nil {
		return "", err
	}
...
// CanonicalWithinRoot 返回解析符号链接后的安全目标路径。
func CanonicalWithinRoot(root, target string) (string, error) {
...
// ConfiguredSkillOutput 根据 skills target 配置解析最终输出路径。
func ConfiguredSkillOutput(projectRoot string, configRepo config.Reader) (string, error) {
	target := ""
	outputPath := ""
	if configRepo != nil {
	...
	return ResolveOutput(projectRoot, outputPath)
}

// Relative 返回相对于 projectRoot 的斜杠分隔路径。
func Relative(projectRoot string, paths []string) []string {
	if len(paths) == 0 {
		return nil
	}
