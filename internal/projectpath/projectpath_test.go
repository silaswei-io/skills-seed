...
func TestFindSeedPath(t *testing.T) {
...
func TestResolveExpandsWindowsStyleHomePrefix(t *testing.T) {
	projectRoot := t.TempDir()

	resolved, err := Resolve(projectRoot, `~\skills-seed`)
...
func TestResolveOutputRejectsPathsOutsideProjectRoot(t *testing.T) {
	parent := t.TempDir()
	projectRoot := filepath.Join(parent, "repo")
	require.NoError(t, os.MkdirAll(projectRoot, 0755))

	inside, err := ResolveOutput(projectRoot, ".agents/skills/demo")
	require.NoError(t, err)
	assert.Equal(t, filepath.Join(projectRoot, ".agents", "skills", "demo"), inside)

	_, err = ResolveOutput(projectRoot, "../outside")
	require.Error(t, err)
	...
	_, err = ResolveOutput(projectRoot, outsidePath)
	require.Error(t, err)
	assert.Contains(t, err.Error(), i18n.GetWithParams("GenerateOutputPathOutsideProjectRoot", map[string]interface{}{
	...
	_, err = ResolveOutput(projectRoot, ".")
	require.Error(t, err)
}
...
func TestResolveOutputRejectsSymlinkEscape(t *testing.T) {
	...
	_, err := ResolveOutput(projectRoot, ".agents/skills/demo")
	require.Error(t, err)
}
...
func TestRelative(t *testing.T) {
	...
	result := Relative(projectRoot, paths)
