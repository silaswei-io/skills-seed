...
func (c *CodexAgent) StartLearningSession(ctx context.Context, req agent.LearningSessionRequest) (agent.LearningSession, error) {
	if sessionID := strings.TrimSpace(req.ResumeSessionID); sessionID != "" {
		return &learningSession{agent: c, sessionID: sessionID}, nil
	}
	...
	output, sessionID, err := c.callCodexSession(ctx, agent.OperationLearningConversationStart, prompt, aicontract.ContractLearningSessionAck, "", task)
	if err != nil {
		return nil, err
	}
	...
	if sessionID == "" {
		return nil, fmt.Errorf("%s", i18n.Get("AgentCodexLearningSessionIDMissing"))
	}
	return &learningSession{agent: c, sessionID: sessionID}, nil
}
...
func (s *learningSession) call(ctx context.Context, operation, templateName, inputPrefix, outputContract string, task agent.RuntimeTask, build func(*agent.PromptInputSession) (map[string]interface{}, error)) (string, error) {
	...
	promptData, err := build(inputs)
	if err != nil {
		return "", err
	}
	return s.callPrepared(ctx, operation, templateName, outputContract, task, promptData)
}
...
func (s *learningSession) callPrepared(ctx context.Context, operation, templateName, outputContract string, task agent.RuntimeTask, data interface{}) (string, error) {
	...
	output, sessionID, err := s.agent.callCodexSession(ctx, operation, prompt, outputContract, s.sessionID, task)
	if err != nil {
		return "", err
	}
	if sessionID == "" {
		return "", fmt.Errorf("%s", i18n.GetWithParams("AgentCodexLearningSessionIDLost", map[string]interface{}{"Operation": operation}))
	}
