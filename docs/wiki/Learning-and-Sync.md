...
- 续跑会重新校验已保存的 Pattern 规范化决策。本轮源码候选先以自己的 Pattern ID 重建来源身份，历史 `MergedFrom` 只保留为既有输出谱系，不会回流为当前输入归属；同 ID 当前候选作为该 Pattern 的当前版本覆盖已有记录。若某条决策确实合并了不同 ID 的能力入口，系统会保留独立候选并覆盖该 checkpoint，不会让错误合并进入模式库或中断入库。
