...
- Candidates explicitly dropped during normalization are recorded in the run journal and changelog with a `reason_code` and short reason, so reviewers can see why they were not stored.
- Focuses with no candidates are marked reviewed locally after analysis. `standard`-depth candidates that pass the local hard gate (complete structure, evidence paths, no `operational_risk`) are accepted locally; `careful`/`critical` and high-risk candidates still use independent AI review.
- When there are no candidates and no retirements, normalize AI is skipped. When candidates have no merge relations, normalize AI short-circuits to deterministic keep. Before store, evidence paths are gated against the project root; unverifiable ownership is dropped as `unsupported_evidence`.
- The run journal stores structured `metrics` (stage timings, agent calls, skipped stages, analysis mode).
