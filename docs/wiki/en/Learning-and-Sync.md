...
- Even when an Agent CLI returns success, the same call applies both dynamic JSON Schema and business-parser validation. Missing fields, invalid enums, incomplete receipts, JSON extraction failures, and provider invocation timeouts follow `agent.retry` without rebuilding the agenda or rerunning completed focuses.
- Independent knowledge review operates on a complete focus, rather than mechanically splitting by candidate count.
- Candidates and the agenda are persisted as soon as planning finishes, and each focus is checkpointed after source analysis and review. On interruption, `--resume` consumes the saved candidates and agenda directly: it does not rerun local candidate selection or agenda planning, and retries only unfinished focuses. A content-hash change in any recorded source, current changes outside the saved scope, or an explicit restart invalidates the old state and replans the agenda. Upgrading the CLI alone does not replan merely because its version changed.
