...
- Resumption revalidates saved Pattern-normalization decisions. A current candidate with the same ID replaces the stored version of that Pattern; if a decision actually merges capability entries from different IDs, the CLI retains separate candidates and replaces that checkpoint instead of admitting the merge or interrupting persistence.
