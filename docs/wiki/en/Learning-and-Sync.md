...
- Focuses with no candidates are marked reviewed locally after analysis and never enter the AI review queue. When there are no candidates and no retirements, normalize AI is skipped while the source baseline still commits. The run journal stores structured `metrics` (stage timings, agent calls, skipped stages, analysis mode).
