...
| Delta analysis reports an omitted evidence focus | Upgrade, then run `skills-seed sync --resume` | Reuses the valid agenda and completed focuses; the runtime Schema requires one receipt per focus, and local admission retains `no_change` when rejecting a candidate |
