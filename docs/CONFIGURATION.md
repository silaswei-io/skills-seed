...
0.9.1 起，skills 生成会读取 dirty state：学习、删除模式或 workspace 关系变化只标记受影响目标待生成；输入未变化且目标不脏时会跳过生成。需要忽略 dirty state 做全量重建时，使用 `skills-seed generate skills --force`。
...
2. 排除规则会影响学习、预览、项目结构摘要、样例文件收集和结构化预扫描。
