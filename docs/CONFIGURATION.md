...
| 字段 | 默认值 | 说明 |
|---|---:|---|
| `mode` | `normal` | 分析深度：`normal` 默认且最稳妥，平衡质量和速度；`fast` 更快，只保留高价值核心模式；`deep` 更深入，会读取更深分支、外部依赖和异常路径 |
| `scope` | `flow` | 单元切分范围：`flow` 默认且最稳定，按业务流程/资源动作拆分；`domain` 更粗、模式更少，适合高层长期规则；`module` 更细，适合模块/插件边界清晰的仓库 |
| `parallelism` | `1` | 项目内分析单元并发数；普通项目和 workspace 子项目生效，`1` 表示串行 |
| `max_units_per_call` | `1` | 单次 AI 调用最多分析的单元数；`1` 表示不合批，降低单次输出过大、解析失败和跨单元结论串扰风险 |
...
0.9.0 起，模式库入库前会执行候选策展。0.13.11 起，`learn current` 使用最小 `pattern-curate` AI 契约，只让 Agent 决定规范文本、置信度和真实 `source_ids`；示例、源码证据、业务方法、频次、来源和统计由本地代码从输入恢复。未知来源会被清理，未分类候选只执行本地确定性恢复，不再完整重跑 AI；同一候选同时被保留和丢弃时，以可追溯的规范模式为准。其它学习入口使用确定性合并，显式 `patterns compact --ai` 保持严格校验。`generate skills` 只读取已保存数据，不执行模式合并或 Agent 调用。
