...
模式库入库前会执行候选规范化。当前 `learn current` 的候选模式来自证据包分析结果，并先经过独立知识审查；本地服务再按配置的证据与置信度门槛准入，AI 只对新旧模式提出语义合并建议。本地服务负责校验来源 ID、从来源传播受控知识标志、确定性处理重复归属、补回遗漏的合格候选并一次性写入。同 ID 当前候选表示该 Pattern 的当前有效版本，会覆盖已有记录后再参与校验和水合；不同 ID 才作为独立来源判断是否允许合并。`generate skills` 只读取已保存数据，不执行模式合并或 Agent 调用。
